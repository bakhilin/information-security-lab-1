...
def get_profile():
    try:
        ...
        return (
            jsonify(user_data) if user_data else jsonify({"error": "User not found"})
        ), 404
    except Exception as e:
        return jsonify({"error": str(e)}), 500
...
def create_post():
    try:
        ...
        post_id = PostModel.create_post(
            data["title"], data["content"], get_jwt_identity()
        )
...
def health_check():
    return (
        jsonify({"status": "healthy", "service": "Flask API", "database": "SQLite3"}),
        200,
    )
